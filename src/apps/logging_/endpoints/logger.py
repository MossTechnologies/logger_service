...
@router.post('/write/', tags=['Frontend logging API', 'User logging API'])
async def write_log(
    log_info: AllLogInfo,
    request: Request,
    ...
    if request.headers['content-type'].lower() == 'application/json':
        try:
            system_info_model = create_system_info_model(
                **log_info.system_info.dict()
            )
            ...
            Logs(project_model, system_info_model).write(log=log_info.dict())
...
@router.get('/get-all-projects/', tags=['Frontend logging API'])
